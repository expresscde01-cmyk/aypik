**Traducción no oficial — con fines meramente informativos.** La versión original en francés de estos Términos de Servicio ("Conditions Générales d'Utilisation") es el único documento jurídicamente vinculante. En caso de discrepancia o conflicto entre esta traducción y la versión francesa, prevalecerá la versión francesa.

---

# Aypik — El sitio de citas para personas sin hijos

## Términos de Servicio

Última actualización: 10 de septiembre de 2026. Versión aplicable al lanzamiento voluntario y gratuito del Servicio.

### Preámbulo

Aypik es una plataforma de citas en línea dedicada exclusivamente a personas adultas que no tienen hijos. Se publica a título personal, como proyecto voluntario y sin ánimo de lucro en su lanzamiento, concebido para seguir siendo accesible sin exigir contraprestación económica alguna para su uso básico.

El Servicio se basa en la transparencia, la amabilidad y el rechazo de los "dark patterns" (patrones oscuros): sin prácticas engañosas, sin compromisos ocultos, sin necesidad de tarjeta bancaria para registrarse o utilizar el núcleo del Servicio, y sin reducción artificial de las funciones básicas destinada a forzar un pago.

### Artículo 1 — Aviso Legal (LCEN)

De conformidad con la Ley francesa n.º 2004-575, de 21 de junio de 2004, para la confianza en la economía digital ("LCEN"), este aviso identifica al editor del Servicio y a su alojador (host).

**Editor del Servicio**

El sitio web Aypik (disponible en particular en https://aypik.fr) es publicado por una persona física que actúa a título no profesional, en el marco de un proyecto voluntario.

Director de publicación: el fundador de Aypik, persona física, contactable en aypik.contact@gmail.com.

De conformidad con el artículo 6, III de la LCEN, dado que el sitio se publica a título no profesional, el editor puede limitar la publicación de sus datos identificativos personales, siempre que estos hayan sido comunicados al alojador. Cualquier reclamación, solicitud de ejercicio de derechos o denuncia puede enviarse a la dirección de correo electrónico indicada anteriormente.

**Alojador (Host)**

o2switch, sociedad por acciones simplificada (SAS) de derecho francés con un capital social de 100.000 €
Chemin des Pardiaux, 63000 Clermont-Ferrand, Francia
Registro Mercantil y de Sociedades (RCS) de Clermont-Ferrand 510 909 807
SIRET 510 909 807 00032
Número de IVA intracomunitario FR35 510 909 807
Sitio web: https://www.o2switch.fr

### Artículo 2 — Objeto, Aceptación y Evolución del Modelo

**2.1 Objeto**

Los presentes Términos de Servicio (los "Términos") definen las condiciones de acceso y uso del Servicio Aypik: creación de perfil, descubrimiento de miembros, expresión de interés (Like, Flash), formación de Matches, mensajería condicionada a un Match y configuración de visibilidad de la cuenta.

Estos Términos no constituyen condiciones generales de venta. En el lanzamiento no se vende ninguna oferta de pago.

**2.2 Aceptación**

Registrarse, crear una cuenta o utilizar el Servicio implica la aceptación plena e íntegra de estos Términos. Si el usuario no acepta estos Términos, debe abstenerse de utilizar el Servicio y puede solicitar la eliminación de su cuenta.

**2.3 Núcleo del Servicio Gratuito**

El núcleo del Servicio —es decir, como mínimo, la creación y gestión de un perfil, la exploración de perfiles compatibles, la expresión de interés básico, la formación de Matches y el uso de la mensajería entre miembros emparejados— seguirá siendo accesible de forma gratuita.

Un usuario que no desee pagar nada no perderá ningún servicio básico. Ninguna función que forme parte del núcleo del Servicio será suprimida, restringida o condicionada a un pago de manera que obligue al usuario a suscribir una opción de pago.

**2.4 Desarrollos Futuros — Posibles Opciones de Pago**

El editor se reserva el derecho de ofrecer, en una fecha posterior y cuando el proyecto lo justifique, módulos u opciones de pago estrictamente opcionales (mayor comodidad, visibilidad aumentada o funciones adicionales que no sean esenciales para el uso básico).

En su caso:

- se informará previamente a los usuarios, de forma clara y distinta;
- estos módulos se regirán por unas Condiciones Generales de Venta ("CGV") separadas de estos Términos, que deberán aceptarse expresamente en el momento de la suscripción;
- rechazar la suscripción a una opción de pago no dará lugar al cierre de la cuenta ni a la pérdida de los servicios básicos descritos en el apartado 2.3.

Mientras dichas CGV no hayan sido publicadas y aceptadas, no se adeuda ningún pago ni se solicita ninguna tarjeta bancaria.

**2.5 Modificación de estos Términos**

El editor podrá modificar estos Términos por motivos legítimos (evoluciones legales, técnicas o del Servicio). La versión aplicable es la versión fechada publicada en el Sitio. En caso de cambio sustancial, se notificará a los usuarios. Continuar utilizando el Servicio tras la entrada en vigor de dichos cambios constituye una aceptación, sin perjuicio del derecho a eliminar la cuenta.

### Artículo 3 — Condiciones de Acceso

A efectos de estos Términos, los términos "sitio" y "aplicación" designan un mismo Servicio Aypik, accesible mediante un navegador en https://aypik.fr o instalado en un dispositivo (como aplicación web progresiva, o PWA).

Cuando el navegador lo permita, el usuario podrá añadir el Servicio a la pantalla de inicio de un teléfono o tableta, o al escritorio de un ordenador. Esta instalación se ofrece directamente en Android, en los navegadores Chrome y Samsung Internet, así como en ordenadores, en los navegadores Chrome y Edge. También es posible, siguiendo las instrucciones mostradas en el Servicio, en iPhone e iPad en el navegador Safari, en Firefox para Android y en Safari para macOS a partir de la versión Sonoma. No se ofrece en Firefox para ordenadores, ni en otros navegadores que carezcan de una opción equivalente para añadir a la pantalla de inicio.

En todos los casos, incluso cuando esta instalación no se ofrezca, el acceso al Servicio mediante un navegador, en https://aypik.fr, permanece plenamente disponible y sin cambios. La instalación es únicamente un acceso directo opcional; nunca constituye una condición de acceso al Servicio, y no modifica su funcionamiento, los datos asociados a la cuenta ni estos Términos, ni da lugar a ninguna recopilación de datos adicional respecto al uso mediante navegador. Dado que el Servicio no se distribuye a través de Google Play ni de la App Store, no se aplican las condiciones de uso de terceros de dichas plataformas.

**3.1 Mayoría de Edad — 18 Años o Más**

El Servicio está exclusivamente reservado para personas adultas. Queda estrictamente prohibido cualquier registro, acceso o uso por parte de una persona menor de 18 años.

Al registrarse, el usuario declara y garantiza tener 18 años o más en la fecha de creación de la cuenta. Se requiere una fecha de nacimiento; se deniega el acceso si la edad declarada es inferior a 18 años.

El editor se reserva el derecho de rechazar, suspender o eliminar, sin previo aviso, cualquier cuenta cuyo titular resulte ser menor de edad o haya proporcionado una fecha de nacimiento inexacta con el fin de eludir esta prohibición.

**3.2 A Quién Está Destinado el Servicio**

El Servicio está destinado a personas adultas que declaran no tener hijos. El usuario se compromete a proporcionar información veraz en su perfil a este respecto. Un perfil manifiestamente incompatible con esta línea editorial podrá ser suspendido o eliminado.

**3.3 Una Cuenta por Persona**

Cada usuario solo puede tener una única cuenta. Se prohíbe crear, mantener o utilizar —de forma simultánea o sucesiva, en incumplimiento de esta norma— varias cuentas por una misma persona.

La apertura de una cuenta está vinculada a una única dirección de correo electrónico, que sirve como identificador único de la cuenta. Se prohíbe crear varias cuentas desde la misma dirección de correo electrónico, o varias cuentas para una misma persona, incluso utilizando direcciones de correo electrónico diferentes.

El editor podrá rechazar el registro, o suspender o eliminar, sin previo aviso ni indemnización, cualquier cuenta que resulte ser un duplicado, un caso de posesión de varias cuentas o un intento de eludir esta norma. En tal caso, el usuario no tendrá derecho a ninguna compensación ni transferencia de ventajas, incluido el estatus de Miembro Fundador y el número asociado.

**3.4 Amabilidad y Moderación**

Los usuarios se comprometen a un uso leal, respetuoso y considerado del Servicio: sin acoso, insultos, comentarios de odio, contenido ilícito, suplantación de identidad, prospección comercial no autorizada ni comportamiento que pueda comprometer la seguridad de terceros.

La mensajería solo se abre tras un Match, con el fin de limitar los contactos no solicitados. Un Like permanece discreto hasta que es correspondido. Los plazos y las etiquetas relativas a los perfiles en espera, así como los estados "Primer mensaje" y "Conversación en curso", se detallan en el Anexo A (Glosario), en aras de la transparencia y para evitar cualquier ambigüedad sobre el cierre de las interacciones.

En el marco de una obligación de medios, el editor podrá moderar, ocultar o eliminar contenido, y advertir, suspender o eliminar una cuenta, sin previo aviso cuando la gravedad de los hechos lo justifique. Puede enviarse una denuncia a aypik.contact@gmail.com.

Un usuario cuya cuenta haya sido suspendida o eliminada por incumplimiento de estos Términos podrá impugnar esta decisión escribiendo a aypik.contact@gmail.com, exponiendo los puntos que desee que se tengan en cuenta. El editor examina estas impugnaciones en un plazo razonable.

**3.5 Género y Criterios de Emparejamiento**

El género (hombre o mujer), que debe indicarse en el registro, determina los criterios de emparejamiento: a una cuenta "hombre" se le muestran perfiles "mujer", y viceversa.

**3.6 Contraseña**

El usuario puede cambiar su contraseña en cualquier momento desde la configuración de su perfil (página Perfil), sin necesidad de contactar con el editor. Es responsabilidad del usuario elegir una contraseña suficientemente robusta y mantenerla confidencial.

**3.7 Descubrir, Sugerencias y Visibilidad de la Cuenta**

*3.7.1 Filtrado de las Sugerencias*

Los perfiles mostrados en Descubrir y en las sugerencias de la página de Inicio se determinan según los criterios de emparejamiento (apartado 3.5) y los mecanismos descritos en el glosario (Like, Flash, Match).

Un perfil al que el usuario ya haya enviado un Like o un Flash deja de mostrarse en Descubrir o en las sugerencias de la página de Inicio. Esta exclusión no elimina la interacción ya registrada: el perfil sigue siendo gestionable desde Mis Matches, según su estado (Por revisar, Match, archivado, etc.).

Cuando un Like o un Flash ha sido rechazado —por el usuario, por el otro miembro, o como resultado de la expiración automática de una puesta en espera—, el perfil en cuestión deja de mostrarse a la otra parte en Descubrir o en las sugerencias de la página de Inicio durante un período de 3 meses a partir de la fecha del rechazo. Esta ocultación se aplica en ambos sentidos. Al finalizar este período, el perfil podrá volver a mostrarse, sujeto a los demás filtros de este apartado y a los criterios de emparejamiento (apartado 3.5).

*3.7.2 Modos de Visibilidad*

El usuario selecciona un único modo de visibilidad a la vez, desde el menú de la cuenta (entrada "Visibilidad"). Las denominaciones siguientes son las que se muestran en dicho menú.

**Normal.** El perfil es visible con normalidad: puede mostrarse en Descubrir y en las sugerencias de la página de Inicio, sujeto a los filtros del apartado 3.7.1 y a las demás condiciones aquí previstas. El estado en línea (apartado 3.8) puede mostrarse a otros miembros.

**Incógnito.** El usuario sigue utilizando el Servicio. Su perfil puede seguir mostrándose en Descubrir y en las sugerencias. Sin embargo, no aparece "en línea" para otros miembros (apartado 3.8). Un usuario en modo Incógnito sigue viendo el estado en línea de otros miembros, cuando corresponda.

**Dejar de aparecer en Descubrir y Sugerencias.** El perfil deja de mostrarse a otros miembros en Descubrir o en las sugerencias de la página de Inicio. Los Matches ya formados, las conversaciones y las interacciones ya registradas (Likes, Flashes) se conservan. El usuario puede seguir utilizando el Servicio, incluidos Mis Matches y la mensajería condicionada a un Match.

**Pausar la cuenta.** La cuenta queda totalmente en pausa. El usuario puede seguir iniciando sesión; el uso del Servicio queda entonces limitado a la reactivación de la cuenta. El perfil deja de mostrarse en Descubrir o en las sugerencias de la página de Inicio mientras dure la pausa. No se registra ningún Like, Flash ni mensaje nuevo para esta cuenta durante la pausa; se informa de ello a cualquier miembro que intente dicha interacción. Los Matches, conversaciones e interacciones ya existentes no se eliminan; vuelven a estar disponibles al reactivar la cuenta. Los Likes, Flashes y mensajes que se hubieran enviado durante la pausa no se conservan.

Una cuenta para la que el usuario haya elegido uno de los modos anteriores, incluido "Pausar la cuenta", sigue siendo una cuenta activa a efectos de estos Términos. El estatus de Miembro Fundador y el número asociado no se pierden. Solo la eliminación definitiva de la cuenta, en las condiciones del apartado 8.2, conlleva la pérdida permanente de esta insignia y de este número.

**3.8 Presencia en Línea**

Cuando un miembro utiliza el Servicio durante una sesión activa, una marca de tiempo de última actividad (last_active_at) puede actualizarse periódicamente. Un perfil se considera "en línea" cuando se detecta actividad reciente (una ventana de unos minutos) y el modo Incógnito no está activado.

Este estado puede mostrarse a otros miembros mediante un indicador visual (punto verde) en la foto de perfil, en particular en Descubrir, en las sugerencias de la página de Inicio y en Mis Matches. La marca de tiempo en sí no se muestra a otros miembros. En modo Incógnito, el perfil nunca se muestra como en línea, aunque el miembro esté efectivamente utilizando el Servicio.

El tratamiento de last_active_at se describe en el apartado 8.1.

### Artículo 4 — Oferta "Miembro Fundador"

La oferta de Miembro Fundador es honorífica. Está estrictamente reservada a los primeros 500 miembros registrados, según disponibilidad. En cuanto se alcance este límite, la oferta dejará de estar disponible. Los números de Miembro Fundador se asignan de forma secuencial y nunca se reasignan.

Durante un período de seis (6) meses a partir de la activación de la oferta en la cuenta, las ventajas funcionales se conceden de forma gratuita, sin compromiso de duración determinada, sin renovación automática y sin que se solicite en ningún momento una tarjeta bancaria. Según las funciones efectivamente desplegadas, incluyen en particular: Likes y Flashes ilimitados, así como un impulso de visibilidad del perfil durante el primer mes.

Al finalizar estos seis meses, las ventajas funcionales terminan. La cuenta permanece entonces en la oferta básica gratuita. El título de Miembro Fundador y el número asociado siguen siendo visibles con carácter honorífico mientras la cuenta esté activa.

El estatus de Miembro Fundador y su número asociado están estrictamente vinculados a la cuenta activa. En caso de cancelación o eliminación de la cuenta, la insignia se pierde de forma permanente y no puede reasignarse. Una cuenta cuya visibilidad esté restringida o que esté en pausa a efectos del apartado 3.7 sigue siendo una cuenta activa; véase también el apartado 8.2.

Esta oferta no constituye un contrato de venta, una suscripción ni una contraprestación económica. No da lugar a ningún reembolso, transferencia o conversión en efectivo.

### Artículo 5 — Boost y Estatus Premium

La insignia "Premium" es la etiqueta visual de las ventajas de Fundador durante su período de activación de 6 meses (Artículo 4): muestra visualmente, en el perfil, que estas ventajas (Likes y Flashes ilimitados, Boost gratuito el primer mes) están actualmente activas. Desaparece al finalizar los 6 meses, a diferencia del título honorífico "Miembro Fundador" y su número, que permanecen mostrados mientras la cuenta esté activa (Artículo 4). La validez del Boost puede consultarse en cualquier momento desde la página "Mi Perfil".

### Artículo 6 — Propiedad Intelectual y Contenido del Usuario

**6.1 Elementos de la Plataforma**

Toda la estructura del Sitio, su código fuente, interfaces, textos, gráficos, bases de datos, logotipos y estilo editorial están protegidos por el derecho de autor, el derecho de marcas y, en su caso, los derechos de productor de bases de datos. Queda prohibida cualquier reproducción, extracción (incluso mediante scraping), adaptación o explotación no autorizada.

**6.2 Marca**

El nombre Aypik, sus variaciones y su identidad visual están protegidos. Queda prohibido cualquier uso no autorizado.

**6.3 Contenido Publicado por el Usuario**

El usuario conserva los derechos que posee sobre el contenido que publica (foto, biografía, intereses, mensajes y, en su caso, testimonio).

Concede al editor, mientras el contenido permanezca en el Servicio, una licencia no exclusiva, mundial, gratuita e intransferible, estrictamente limitada a lo necesario para el funcionamiento del Sitio: alojamiento, visualización a los miembros correspondientes, almacenamiento en caché, copia de seguridad técnica y, en su caso, moderación.

Esta licencia finaliza, salvo copias de seguridad técnicas transitorias, con la eliminación del contenido o de la cuenta. El usuario garantiza que posee los derechos necesarios y que su contenido no vulnera los derechos de terceros.

### Artículo 7 — Responsabilidad

Aypik es una herramienta de puesta en contacto. El editor no es parte en las relaciones que se establecen entre los usuarios y no organiza encuentros fuera de línea.

El usuario es el único responsable de la información que proporciona, de la exactitud de su perfil y de sus intercambios y encuentros. Corresponde al usuario actuar con precaución (no compartir datos bancarios, preferir un lugar público para un primer encuentro, etc.). El editor no podrá ser considerado responsable de la conducta de los miembros fuera de línea, de la ausencia de un encuentro, ni de la inexactitud del perfil de un tercero, siempre que el editor no haya tenido conocimiento efectivo de ello.

El editor, que actúa a título voluntario, asume una obligación de medios: continuidad razonable del Servicio, seguridad adecuada a un sitio de esta naturaleza y moderación diligente de las denuncias. El Servicio se presta "tal cual"; pueden producirse interrupciones (mantenimiento, fuerza mayor, fallo de un proveedor).

La responsabilidad del editor solo podrá comprometerse en caso de falta probada que le sea directamente imputable. Se limita, en la medida permitida por la ley, a los daños directos. Nada de lo aquí dispuesto excluye la responsabilidad en caso de dolo o culpa grave, ni la indemnización por daños corporales.

En relación con el contenido ilícito denunciado, el editor actúa de conformidad con la LCEN en cuanto tenga conocimiento efectivo del mismo.

### Artículo 8 — Datos Personales, Baja y Ley Aplicable

**8.1 Protección de Datos (RGPD)**

El editor es responsable del tratamiento de los datos personales recabados para prestar el Servicio (cuenta, perfil, interacciones, mensajes, número de teléfono cuando se utiliza la verificación por SMS, la marca de tiempo last_active_at mencionada en el apartado 3.8 y los registros técnicos necesarios para la seguridad). El tratamiento se basa principalmente en la ejecución del contrato de uso (estos Términos) y, en su caso, en el consentimiento (por ejemplo, las notificaciones por correo electrónico o un testimonio) o en el interés legítimo (seguridad, prevención del fraude y de las cuentas múltiples).

Los datos no se venden. Solo pueden compartirse con proveedores estrictamente necesarios (alojador, infraestructura técnica, envío de correos electrónicos y SMS de verificación), establecidos en la Unión Europea u que ofrezcan garantías adecuadas.

**Cookies y rastreadores.** El Servicio utiliza actualmente solo cookies estrictamente necesarias para su funcionamiento (en particular, el mantenimiento de la sesión de inicio de sesión), para las cuales no se requiere consentimiento previo. Si el editor llegara a introducir, en el futuro, cookies o rastreadores no esenciales (por ejemplo, con fines de medición de audiencia), se informaría al usuario mediante un banner específico que le permitiría aceptar, rechazar o personalizar su consentimiento antes de que se instalen dichas cookies, de conformidad con la normativa aplicable. En tal caso, se publicaría una política de cookies específica, accesible desde el Sitio.

El usuario tiene derecho a acceder, rectificar, suprimir, limitar y oponerse al tratamiento de sus datos, así como derecho a la portabilidad de los datos y derecho a presentar una reclamación ante la CNIL (www.cnil.fr), la autoridad de control francesa. Estos derechos pueden ejercerse a través de la configuración del perfil y/o en aypik.contact@gmail.com.

**8.2 Baja y Eliminación de la Cuenta**

El usuario puede solicitar en cualquier momento, desde la configuración de su perfil, la eliminación de su cuenta y de todos sus datos personales.

Los modos de visibilidad descritos en el apartado 3.7, incluida la opción "Pausar la cuenta", no constituyen una eliminación de la cuenta. El perfil y los datos se conservan; la cuenta permanece activa. La pérdida del estatus de Miembro Fundador y del número asociado solo se produce en caso de cancelación o eliminación en virtud de este apartado 8.2.

Para el usuario, esta eliminación es definitiva: el perfil deja de ser visible y de utilizarse. Por motivos técnicos y de seguridad (corrección de un error de tratamiento, obligaciones legales), los datos pueden conservarse internamente durante un período de purga de treinta (30) días, transcurrido el cual se borran de forma irrecuperable. Determinados registros estrictamente necesarios (por ejemplo, futuras facturas, o conservación exigida por la ley) podrán, en su caso, conservarse durante más tiempo, de forma separada.

El estatus de Miembro Fundador y su número asociado están estrictamente vinculados a la cuenta activa. En caso de cancelación o eliminación de la cuenta, la insignia se pierde de forma permanente y no puede reasignarse.

El editor podrá dar de baja una cuenta en caso de incumplimiento grave o reiterado de estos Términos.

**8.3 Ley Aplicable y Tribunales Competentes**

Estos Términos se rigen por el derecho francés.

En caso de litigio, y tras un intento de resolución amistosa (contacto: aypik.contact@gmail.com), serán competentes los tribunales franceses. Cuando el usuario tenga la condición de consumidor, podrá interponer la acción, además de ante los tribunales territorialmente competentes conforme al Código de Procedimiento Civil francés, ante el tribunal del lugar en el que residía en el momento de celebrarse el contrato o de producirse el hecho dañoso. También podrá recurrir a un mediador de consumo, en las condiciones previstas por el Código de Consumo francés, cuando dicha opción esté disponible para él.

### Anexo A — Glosario

**Mis Matches.** Página que reúne las interacciones del usuario (intereses recibidos, Matches, archivos y secciones relacionadas). En esta página, los perfiles se ordenan cronológicamente según su aparición: los añadidos más recientemente aparecen en la parte superior izquierda de la cuadrícula, mientras que los perfiles anteriores se desplazan progresivamente hacia la parte inferior derecha, siguiendo el orden de lectura habitual (de izquierda a derecha y de arriba abajo).

**Like.** Una acción sencilla que permite expresar discretamente interés por el perfil de otro miembro.

**Flash.** Una acción prioritaria, más fuerte que un simple Like, que envía una señal clara y directa a la persona destinataria para indicar un interés inmediato.

**Match.** Una relación recíproca establecida entre dos miembros, que confirma que se ha establecido un interés mutuo (ya sea por Likes cruzados o por un Flash aceptado). Es esta confirmación mutua la que abre el acceso a la mensajería.

**Mensajería y Consentimiento.** Como medida de seguridad y para evitar contactos no solicitados, el envío de mensajes está estrictamente condicionado a un Match previo. Por tanto, no es posible conversar con un miembro sin haber recibido, o haber visto aceptado a cambio, su Like/Flash.

**Por Revisar.** El estado de un perfil cuyo Like o Flash se ha recibido y aún no ha sido decidido. El usuario puede entonces rechazar el perfil, ponerlo en espera o confirmar un Match.

**Puesta en Espera.** Una acción que permite conservar un perfil recibido (Like o Flash) para revisarlo más tarde, sin rechazarlo ni formar un Match de inmediato. El perfil permanece entonces accesible desde Mis Matches, en "Puestos en espera por ti", hasta que se cierre la espera: mediante un Match, un rechazo o la expiración automática tras 3 meses de inactividad. Archivar no constituye un cierre: es una acción de organización personal, sin efecto sobre la interacción ni sobre el otro miembro. El estado permanece en espera, el plazo de expiración sigue corriendo y no se envía ninguna notificación al otro miembro. Los perfiles archivados de este modo siguen siendo visibles en "Puestos en espera por ti - archivados". A la inversa, cuando otro miembro pone en espera el Like o el Flash del usuario, este perfil aparece para el usuario en "Puestos en espera por ellos", solo para consulta: la decisión de esperar, formar un Match o rechazar corresponde al otro miembro.

Un perfil en estado de "espera" (ya sea por parte del usuario o del otro miembro) sin una decisión explícita (aceptación o rechazo) se considera automáticamente rechazado tras un período de 3 meses de inactividad. Archivar un perfil en espera no constituye dicha decisión y no interrumpe el transcurso de este plazo. Se envía un recordatorio 7 días antes de la expiración para permitir decidir sobre las solicitudes pendientes. Este cierre automático produce los mismos efectos que un rechazo manual, en ambos sentidos, de modo que ninguna interacción quede indefinidamente pendiente. El rechazo así formado, ya sea manual o automático, también activa la ocultación temporal de Descubrir y de las sugerencias de la página de Inicio descrita en el apartado 3.7.1.

**Match Contigo.** Etiqueta que se muestra cuando es el usuario conectado quien confirma o acepta una solicitud entrante de un tercero (un Like recibido, un Flash recibido o un interés pendiente decidido mediante un Match). La fecha mostrada es la de dicha aceptación ("Match contigo el [fecha]"). En Mis Matches, esta etiqueta puede ir seguida de "— Primer mensaje" o "— Conversación en curso" según el estado de la mensajería.

**Match Con.** Etiqueta que se muestra cuando el Match resulta de que el otro miembro ha aceptado la solicitud inicial del usuario conectado (un Like o un Flash enviado y luego aceptado a cambio). La fecha mostrada es la de dicha aceptación ("Match con [nombre] el [fecha]"). En Mis Matches, esta etiqueta también puede ir seguida de "— Primer mensaje" o "— Conversación en curso" según el estado de la mensajería.

**Primer Mensaje.** Una conversación permanece en estado "Primer mensaje" mientras solo uno de los dos miembros haya iniciado el intercambio sin respuesta de la otra parte, o mientras aún no se haya enviado ningún mensaje. La mensajería está abierta; el intercambio aún no es recíproco.

**Conversación en Curso.** Una conversación pasa al estado "Conversación en curso" únicamente una vez que cada uno de los dos miembros ha enviado al menos un mensaje, lo que marca un intercambio recíproco.

**Archivar / Eliminar.** Opciones de gestión ofrecidas para los perfiles que han rechazado un Like o un Flash, así como para los perfiles en espera. Archivar conserva el perfil en una sección específica de la página Mis Matches. Para una puesta en espera, archivar es únicamente una acción de organización personal: no cierra la interacción, no interrumpe el transcurso del plazo de expiración y no se notifica al otro miembro (véase "Puesta en Espera"). Eliminar retira el perfil de esta lista; para una puesta en espera, la eliminación definitiva equivale a un rechazo.

**Matches Finalizados.** Un espacio de almacenamiento para los Matches archivados o finalizados a partir de una conversación ya confirmada, clasificados según quién tomó la decisión. Si el propio usuario inició el archivado o la finalización, el Match aparece en "Matches que has finalizado": el usuario puede entonces restaurar el vínculo (y el acceso a la mensajería) o eliminarlo definitivamente. Si fue la otra persona quien finalizó el vínculo, el Match aparece en "Matches que ellos finalizaron": solo es posible la eliminación definitiva, ya que no se ofrece la opción de restaurar.

**Boost.** Una función que permite situar el perfil en la parte superior de la lista durante un período determinado para maximizar su visibilidad.

**Descubrir.** La página que permite explorar perfiles compatibles, según los criterios de emparejamiento y el filtrado descritos en el apartado 3.7.1.

**Sugerencias.** Perfiles mostrados en la página de Inicio. Siguen el mismo filtrado que Descubrir (apartado 3.7.1) y los modos de visibilidad del apartado 3.7.2.

**Visibilidad.** Una configuración de cuenta de selección única, accesible desde el menú de la cuenta. Los cuatro modos son: Normal, Incógnito, Dejar de aparecer en Descubrir y Sugerencias, y Pausar la cuenta (apartado 3.7.2).

**En línea.** Un indicador (punto verde en la foto) que muestra que un miembro tiene actividad reciente y no está en modo Incógnito. Véase el apartado 3.8.

### Anexo B — Preguntas Frecuentes

**¿Puedo registrarme si soy menor de 18 años?**
No. El Servicio está exclusivamente reservado para personas adultas.

**¿Puedo crear varias cuentas?**
No. Una cuenta por persona y por dirección de correo electrónico, conforme al apartado 3.3.

**¿El Servicio se convertirá en un servicio de pago?**
El núcleo del Servicio seguirá siendo gratuito. Es posible que se ofrezcan más adelante opciones de pago; serán opcionales, se anunciarán con antelación y se regirán por unas Condiciones Generales de Venta separadas. Quien no pague nada conserva los servicios básicos.

**¿Cómo funciona la eliminación de mi cuenta?**
Se solicita desde tu perfil. El perfil deja de ser visible. Los datos se borran de forma irrecuperable tras un período de purga de 30 días (apartado 8.2).

**¿Qué ocurre con la insignia de Miembro Fundador si cancelo mi cuenta?**
El estatus de Miembro Fundador y su número asociado están estrictamente vinculados a la cuenta activa. En caso de cancelación o eliminación de la cuenta, la insignia se pierde de forma permanente y no puede reasignarse.

**¿Puede reaparecer en Descubrir un perfil al que ya le he dado Like o Flash?**
No. Deja de mostrarse en Descubrir o en las sugerencias de la página de Inicio (apartado 3.7.1). La interacción sigue siendo gestionable desde Mis Matches.

**¿Puede reaparecer en Descubrir un perfil cuyo Like o Flash fue rechazado?**
Sí, pero solo transcurrido un período de 3 meses desde el rechazo (apartado 3.7.1), y sujeto a los demás filtros de sugerencias. Este plazo se aplica tanto si el rechazo fue expresado por el usuario como por el otro miembro, o si resultó de la expiración automática de una puesta en espera.

**¿Cuál es la diferencia entre pausar mi cuenta y eliminar mi cuenta?**
"Pausar la cuenta" (apartado 3.7) deja la cuenta activa: la insignia de Miembro Fundador y su número se conservan; el perfil deja de mostrarse en Descubrir o en las sugerencias, y no se registran nuevas interacciones mientras dure la pausa. La eliminación (apartado 8.2) es definitiva: el perfil deja de utilizarse, los datos se borran tras el período de purga y se pierde la insignia de Fundador.

**¿Qué significa el punto verde en una foto de perfil?**
Indica que se considera que el miembro está en línea (apartado 3.8). Este punto no aparece si dicho miembro ha activado el modo Incógnito.

---

¿Tienes preguntas? [Contáctanos](https://aypik.fr/contact)

Al utilizar Aypik — El sitio de citas para personas sin hijos — confirmas que has leído y aceptado estos Términos de Servicio.

---

**Traducción no oficial — con fines meramente informativos.** Solo la versión en francés es jurídicamente vinculante. En caso de conflicto, prevalecerá la versión francesa.

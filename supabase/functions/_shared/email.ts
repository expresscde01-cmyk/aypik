...
const FALLBACK_FROM = "Aypik <onboarding@resend.dev>";

function stripSenderDisplayNameQuotes(raw: string): string {
  let value = raw.trim();
  if (
...
/** Nom d'expéditeur sans guillemets. L'adresse est lue dans RESEND_FROM_EMAIL. */
export function resendFromAddress(): string {
  const raw = Deno.env.get("RESEND_FROM_EMAIL")?.trim();
  if (!raw) return FALLBACK_FROM;
  return stripSenderDisplayNameQuotes(raw);
}

-- Minimisation profiles (étape 1) : RPC additive, aucun REVOKE.
-- À coller dans l’éditeur SQL Supabase AVANT le déploiement du front.
-- Les colonnes sensibles restent lisibles via GRANT jusqu’à la migration B.

-- ===== my_profile : lecture de SA propre ligne (SECURITY DEFINER) =====
...
AS $$
  SELECT
    ...
    p.created_at,
    p.updated_at,
    public.profile_age(p.birth_date)
...
COMMENT ON FUNCTION public.my_profile() IS
  'Profil du compte connecté uniquement (auth.uid()). Colonnes privées : birth_date, lat/lng, flags compte, e-mail, locale.';

-- ===== my_account_flags : + deletion_requested_at (plus de SELECT PostgREST) =====
...
SET search_path = public
AS $$
  SELECT p.paused_at, p.incognito_at, p.deactivated_at, p.deletion_requested_at
  FROM public.profiles p
  WHERE p.id = (SELECT auth.uid());
$$;
...
REVOKE EXECUTE ON FUNCTION public.my_account_flags() FROM anon;

-- ===== card_profiles : plus de lat/lng/birth_date/deletion_requested_at ; + age =====
DROP FUNCTION IF EXISTS public.card_profiles(uuid[]);

CREATE FUNCTION public.card_profiles(p_ids uuid[])
RETURNS TABLE (
  id uuid,
  display_name text,
  bio text,
...
REVOKE EXECUTE ON FUNCTION public.card_profiles(uuid[]) FROM anon, public;

COMMENT ON FUNCTION public.card_profiles(uuid[]) IS
  'Fiches publiques : âge calculé (profile_age), is_online. Pause/désactivation non filtrées (Mes Matchs inchangé).';

-- ===== suggest_profiles : plus de birth_date ni last_active_at en sortie =====
...
GRANT EXECUTE ON FUNCTION public.suggest_profiles(integer, boolean, integer, text, text, numeric, text, timestamp with time zone, uuid[], text[], text[], text, text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.suggest_profiles(integer, boolean, integer, text, text, numeric, text, timestamp with time zone, uuid[], text[], text[], text, text[]) TO service_role;
REVOKE EXECUTE ON FUNCTION public.suggest_profiles(integer, boolean, integer, text, text, numeric, text, timestamp with time zone, uuid[], text[], text[], text, text[]) FROM anon, public;

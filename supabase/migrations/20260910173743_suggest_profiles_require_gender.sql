-- Retire le contournement du filtre homme/femme quand le visiteur n'a pas
-- renseigné son genre (my_gender IS NULL OR ...). Un compte sans genre
-- défini ne remonte désormais plus aucun profil en suggestion, au lieu de
-- voir tout le monde sans distinction. Ne touche à aucune donnée existante
-- (les profils sans genre restent tels quels ; ils cesseront simplement
-- d'apparaître en résultat de suggest_profiles tant que le champ n'est
-- pas complété). Le verrou "obligatoire à l'inscription" côté formulaire

...
CREATE FUNCTION public.card_profiles(p_ids uuid[])
 RETURNS TABLE(id uuid, display_name text, birth_date date, bio text, has_children boolean, location text, interests text[], photo_url text, gender text, lat double precision, lng double precision, deletion_requested_at timestamp with time zone, is_online boolean, discover_mode text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$

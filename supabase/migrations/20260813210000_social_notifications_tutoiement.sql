-- Tutoiement des notifications in-app (social_notifications).
-- Les pages légales restent en vouvoiement (hors SQL).
...
BEGIN
  ...
  INSERT INTO social_notifications (
    ...
    actor_name || ' t''a envoyé un coup de cœur ✨',
    me,
    inserted.id
  )
...
-- Textes déjà stockés : remplacement sûr des formules connues.
UPDATE social_notifications
SET body = replace(body, ' vous a envoyé un coup de cœur', ' t''a envoyé un coup de cœur')
WHERE body LIKE '% vous a envoyé un coup de cœur%';

UPDATE social_notifications
SET body = replace(body, ' vous a aussi liké — vous pouvez discuter.', ' t''a aussi liké — tu peux discuter.')
WHERE body LIKE '% vous a aussi liké%';

UPDATE social_notifications
SET body = replace(body, ' a liké en retour — vous pouvez discuter.', ' a liké en retour — tu peux discuter.')
WHERE body LIKE '%vous pouvez discuter%';

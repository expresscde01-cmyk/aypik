-- Hexagone / La France dans le monde + continents International (p_world_zones).
-- Ne pas réécrire 20260908140000 : déjà appliquée en prod.

ALTER TABLE public.world_countries
  ADD COLUMN IF NOT EXISTS is_french_territory boolean NOT NULL DEFAULT false;

UPDATE public.world_countries
SET is_french_territory = (iso2 IN (
  'FR', 'GP', 'MQ', 'GF', 'RE', 'YT', 'PF', 'NC', 'WF', 'PM', 'BL', 'MF', 'TF'
));

COMMENT ON COLUMN public.world_countries.is_french_territory IS
  'Métropole + territoires français d''outre-mer (tous statuts).';
...
COMMENT ON FUNCTION public.suggest_profiles(integer, boolean, integer, text, text, numeric, text, timestamptz, uuid[], text[]) IS
  'Accueil / Découvrir. Hexagone : country_code FR. La France dans le monde : territoires FR. International : continents (p_world_zones).';

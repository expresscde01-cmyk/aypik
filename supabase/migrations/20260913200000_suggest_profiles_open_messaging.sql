-- Bouton Dialogue Découvrir : statut ouvert/protégé = is_open_messaging_profile
-- (offre du destinataire), pas discover_mode. discover_mode reste renvoyé
-- (densité Mes Matchs du visiteur, lue ailleurs sur son propre profil).
-- UTF-8. DROP obligatoire : CREATE OR REPLACE ne peut pas ajouter une colonne OUT.
...
COMMENT ON COLUMN public.profiles.discover_mode IS
  'Préférence d’affichage Mes Matchs : "detaille" (parcours complet) ou "simplifie" (uniquement discussions déjà engagées des deux côtés). Sans effet sur l’ouverture du Dialogue ; celle-ci suit is_open_messaging_profile (offre du destinataire).';

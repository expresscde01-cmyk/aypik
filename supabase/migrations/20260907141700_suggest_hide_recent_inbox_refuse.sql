-- Masque Découvrir / Accueil : inbox refuse < 3 mois, dans les deux sens.
-- Pas de rubrique visible. Passé 3 mois, le profil peut réapparaître.

CREATE INDEX IF NOT EXISTS inbox_responses_refuse_user_updated_idx
  ON public.inbox_responses (user_id, updated_at DESC)
  WHERE decision = 'refuse';

CREATE INDEX IF NOT EXISTS inbox_responses_refuse_actor_updated_idx
  ON public.inbox_responses (actor_id, updated_at DESC)
  WHERE decision = 'refuse';
...
REVOKE ALL ON FUNCTION public.suggestion_refuse_hidden_ids(uuid) FROM PUBLIC;
...
COMMENT ON FUNCTION public.suggest_profiles(integer, boolean, integer, text, text, numeric, text, timestamptz, uuid[]) IS
  'Accueil / Découvrir. Masque aussi les refus inbox < 3 mois (les deux sens). is_online : last_active < 5 min et pas Incognito.';

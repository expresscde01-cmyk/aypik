-- Verrou serveur Découvrir : applique offer_clamp_discover_args.
-- UTF-8 (ne pas réécrire via PowerShell Set-Content : casse Île-de-France).
-- RETURNS TABLE aligné sur la production : conserver discover_mode (sinon 42P13
-- ou régression silencieuse du mode fiche simplifié / détaillé).
...
BEGIN
  ...
  excluded AS (
    ...
    UNION
    -- "PassÃ©s" en DÃ©couvrir : 2 mois.
    SELECT dp.to_user
    FROM public.discovery_passes dp
    WHERE dp.from_user = me
      AND dp.created_at >= (now() - interval '2 months')
    UNION
    -- Matchs rompus : 1 an si au moins un message Ã©changÃ©, sinon 6 mois.
    SELECT mb.peer_id

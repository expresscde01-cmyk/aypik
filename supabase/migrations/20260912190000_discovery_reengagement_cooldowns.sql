-- Cooldowns de réapparition dans Découvrir, par niveau d'investissement :
--   - Profil "Passé" (bouton Passer en Découvrir)          : 2 mois
--   - Refus de match (Refuser dans Matches/Inbox)           : 6 mois (au lieu de 3)
--   - Match rompu sans messages échangés                    : 6 mois
--   - Match rompu avec au moins un message échangé          : 1 an

-- 1) Nouvelle table dédiée aux "Passés" de Découvrir (distincte des refus de match :
--    un passage en revue n'est pas un vrai refus, donc pas mélangé à inbox_responses).
...
'Profils "Passés" en Découvrir (bouton Passer). Exclu des suggestions pendant 2 mois après le dernier passage ; effacé avec le profil (RGPD, ON DELETE CASCADE).';

ALTER TABLE public.discovery_passes ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS discovery_passes_insert_own ON public.discovery_passes;
CREATE POLICY discovery_passes_insert_own ON public.discovery_passes
  FOR INSERT
  WITH CHECK ((SELECT auth.uid()) = from_user);

DROP POLICY IF EXISTS discovery_passes_select_own ON public.discovery_passes;
CREATE POLICY discovery_passes_select_own ON public.discovery_passes
  FOR SELECT
  USING ((SELECT auth.uid()) = from_user);

DROP POLICY IF EXISTS discovery_passes_update_own ON public.discovery_passes;
CREATE POLICY discovery_passes_update_own ON public.discovery_passes
  FOR UPDATE
  USING ((SELECT auth.uid()) = from_user)
  WITH CHECK ((SELECT auth.uid()) = from_user);

DROP POLICY IF EXISTS discovery_passes_delete_own ON public.discovery_passes;
CREATE POLICY discovery_passes_delete_own ON public.discovery_passes
  FOR DELETE
  USING ((SELECT auth.uid()) = from_user);

GRANT SELECT, INSERT, UPDATE, DELETE ON public.discovery_passes TO authenticated;
REVOKE ALL ON public.discovery_passes FROM anon;

-- 2) Refus de match (inbox_responses) : délai porté de 3 à 6 mois.
CREATE OR REPLACE FUNCTION public.suggestion_refuse_hidden_ids(p_viewer uuid)
 RETURNS TABLE(hidden_id uuid)
 LANGUAGE plpgsql
...
BEGIN
  ...
  WHERE ir.user_id = p_viewer
    AND ir.decision = 'refuse'
    AND ir.updated_at >= (now() - interval '6 months')
  UNION
  SELECT ir.user_id
  FROM public.inbox_responses ir
  WHERE ir.actor_id = p_viewer
    AND ir.decision = 'refuse'
    AND ir.updated_at >= (now() - interval '6 months');
END;
$function$;

-- 3) suggest_profiles : ajoute l'exclusion "Passés" (2 mois) et "Matchs rompus"
--    (6 mois sans messages échangés, 1 an s'il y a eu au moins un message).

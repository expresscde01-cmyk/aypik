-- International : p_international_countries (plusieurs pays précis).
-- p_international_country (texte) reste un repli pour l’ancien client.

DROP FUNCTION IF EXISTS public.suggest_profiles(integer, boolean, integer, text, text, numeric, text, timestamptz, uuid[], text[], text[], text);
...
CREATE OR REPLACE FUNCTION public.suggest_profiles(
  ...
  p_international_countries text[] DEFAULT NULL
)
RETURNS TABLE (
  id uuid,
  display_name text,
...
COMMENT ON FUNCTION public.suggest_profiles(integer, boolean, integer, text, text, numeric, text, timestamptz, uuid[], text[], text[], text, text[]) IS
  'Accueil / Découvrir. Hexagone : FR. France dans le monde : p_france_world_codes. International : p_world_zones ou p_international_countries.';

REVOKE ALL ON FUNCTION public.suggest_profiles(integer, boolean, integer, text, text, numeric, text, timestamptz, uuid[], text[], text[], text, text[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.suggest_profiles(integer, boolean, integer, text, text, numeric, text, timestamptz, uuid[], text[], text[], text, text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.suggest_profiles(integer, boolean, integer, text, text, numeric, text, timestamptz, uuid[], text[], text[], text, text[]) TO service_role;

...
DO $$ BEGIN
  RAISE EXCEPTION 'OBSOLÈTE : ne pas coller COLLER-GEO-MACRO-ZONES.sql. Source : supabase/migrations/20260910173743_suggest_profiles_require_gender.sql';
END $$;
...
ALTER TABLE public.region_macro_zones DROP CONSTRAINT IF EXISTS region_macro_zones_zone_check;
ALTER TABLE public.region_macro_zones
  ADD CONSTRAINT region_macro_zones_zone_check
  CHECK (zone IN ('northeast', 'northwest', 'center', 'southeast', 'southwest', 'ile_de_france'));

TRUNCATE public.region_macro_zones;
...
COMMENT ON FUNCTION public.region_macro_zone(text) IS
  'Quartier macro (northwest / northeast / center / southwest / southeast) d’une région.';

...
-- Ce collage redéfinit suggest_profiles SANS le masquage 3 mois des refus (suggestion_refuse_hidden_ids).
-- Archive lisible uniquement. Coller ce script est bloqué ci-dessous.
...
/*
-- Ancien contenu (ne pas exécuter) :
-- Pause profil : exclu de Découvrir et des suggestions Accueil, données conservées.
-- Les matchs et conversations existants restent visibles.

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS paused_at timestamptz;

COMMENT ON COLUMN public.profiles.paused_at IS
  'Non NULL = profil en pause, masqué de la découverte et des suggestions. Aucune donnée n''est supprimée.';

CREATE INDEX IF NOT EXISTS profiles_paused_at_alive_idx
  ON public.profiles (created_at DESC)
  WHERE paused_at IS NULL AND deletion_requested_at IS NULL;

-- Dépendance de suggest_profiles (adjacence régions). Absente si
-- COLLER-GEO-MACRO-ZONES n’a pas été exécuté : l’ancienne RPC lisait
-- la table region_neighbors.
